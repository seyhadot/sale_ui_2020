    ...
    <el-row>
      ...
      <el-row>
        <el-table :data="tableData.filter((data) => !search || data.name.toLowerCase().includes(search.toLowerCase()))" style="width: 100%;" @row-click="handleClick">
          ...
          <el-table-column :label="$t('category.tableData.name')" prop="name"></el-table-column>
          <el-table-column :label="$t('category.tableData.desc')" prop="desc"></el-table-column>
          <el-table-column :label="$t('category.tableData.status')">
            <template slot-scope="scope">{{ scope.row.isUsable ? 'បើកអោយប្រើប្រាស់' : 'បិទមិនអោយប្រើប្រាស់' }}</template>
          </el-table-column>
          <el-table-column align="right">
            <template slot="header" slot-scope="scope">
              <el-input v-model="search" size="mini" :placeholder="$t('category.tableData.search')" />
            </template>
          </el-table-column>
        </el-table>
      </el-row>
