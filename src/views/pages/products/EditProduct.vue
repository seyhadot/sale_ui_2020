<template>
  <el-row :gutter="40" v-show="true" class="px-10 h-full">
    <el-form :model="addProduct" :rules="rules" ref="addProductForm" class="flex flex-col h-full justify-between">
        ...
        <el-row :gutter="40" class="mt-3">
          ...
          <el-col :span="12">
            <el-form-item :label="$t('product.form.category')">
              <el-select style="width: 100%;" v-model="addProduct.categoryId" :placeholder="$t('product.form.category')">
                <el-option style="width: 100%;" v-for="item in categoryOptions" :key="item.value" :label="item.label" :value="item.value"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          ...
          <el-col :span="12">
            <el-form-item label="Sub category">
              <el-select style="width: 100%;" multiple clearable filterable v-model="addProduct.subCategories" placeholder="Sub categories">
                <el-option style="width: 100%;" v-for="item in categoryOptions" :key="item.value" :label="item.label" :value="item.value"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          ...
          <el-col :span="12">
            <el-form-item>
              ...
              <br />
              <el-radio v-model="addProduct.type" v-for="(o, i) in productTypes" :key="i" :label="o">{{ o.toUpperCase() }}</el-radio>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item :label="$t('product.form.barcode')">
              <el-select v-model="addProduct.barcodes" multiple filterable allow-create default-first-option placeholder="Add barcodes">
          ...
          <el-col :span="12" v-show="addProduct.type == 'stock'">
            <el-form-item :label="$t('product.form.expired')" prop="expiredAt">
              <el-date-picker v-model="addProduct.expiredAt" type="datetime" :picker-options="options" :placeholder="$t('product.form.expired')"></el-date-picker>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
...
export default {
  ...
  watch: {
    product(val) {
      ...
      if (val) this.getProductById(val._id)
      this.id = val._id
    },
